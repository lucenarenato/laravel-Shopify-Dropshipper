...
@if(isset($faqs_page_contents) && count($faqs_page_contents)>0)
<div class="container">
    <h3>FAQ</h3>
    @foreach($faqs_page_contents as $key=>$record)

        <H4>{{ $record->faq_type }}</h4>

            @if(count($record->cmsFaqContent)>0)
            <div class="accordion" id="accordionExample">
                @foreach($record->cmsFaqContent as $key=>$field)
                     <div class="card">
                        <div class="card-header" id="heading{{$field->id}}">
                            <h2 class="mb-0">
                                <button class="btn btn-link" type="button" data-toggle="collapse" data-target="#collapse{{$field->id}}" aria-expanded="true" aria-controls="collapse{{$field->id}}">
                                    {{ $field->faq_question }}
                                </button>
                            </h2>
                        </div>

                        <div id="collapse{{$field->id}}" class="collapse" aria-labelledby="heading{{$field->id}}" data-parent="#accordionExample">
                            <div class="card-body">
                                {!! $field->faq_answer !!}
                            </div>
                        </div>
